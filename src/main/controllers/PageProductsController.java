...
public class PageProductsController implements Initializable {
    ...
    public void getProductList(String structure){
        ...
        if (structure.equals("all")){
            ...
            // sql command
            cmd = "SELECT * FROM `product` WHERE category IN ('Produce', 'Cleaners', 'Paper', 'Personal Care')";
        } else if (structure.equals("sns")) {
            snsStack = new ArrayStack<>();

            // sql command
            cmd = "SELECT * FROM `product` WHERE category IN ('Frozen', 'Meat', 'Produce', 'Cleaners', 'Paper', 'Personal Care')";
        }
